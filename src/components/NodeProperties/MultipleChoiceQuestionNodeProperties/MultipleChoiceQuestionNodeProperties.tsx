        ...
        <Card className="flex flex-col">
          <TextField
            ...
            onChange={textInputNodeUpdater(
              updater<MultipleChoiceQuestionNode>()('data.question')
            )}
          />
          <Separator />
          <div className="mb-3 flex flex-row justify-between">
            <Text variant="xLarge">Choices</Text>
            <Text className="pr-[44px] self-end" variant="xLarge">
              Is Correct
            </Text>
          </div>
          <Stack tokens={{ childrenGap: 10 }}>
            {selectedNode.data.choices.map((choice, index) => {
              function updateChoice(
                newValue: string
              ): PartialDeep<MultipleChoiceQuestionNode> {
                return produce(selectedNode, (draft) => {
                  draft.data.choices[index] = newValue;
                });
              }

              function updateCorrectChoice(
                checked: boolean
              ): PartialDeep<MultipleChoiceQuestionNode> {
                return produce(selectedNode, (draft) => {
                ...
                <StackItem key={index}>
                  <div className="flex flex-row">
                    ...
                    <IconButton
                      ...
                      iconProps={{
                        iconName: 'Delete',
                        style: { fontSize: 20, color: theme.palette.red },
                      }}
          ...
          <PrimaryButton
            text="Add choice"
            className="mt-4 self-center w-1/2"
            onClick={buttonNodeUpdater(addChoice)}
          />
        </Card>
      </StackItem>
    </Stack>
