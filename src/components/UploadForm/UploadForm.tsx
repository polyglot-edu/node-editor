      ...
      tmp.forEach(async (file) => {
        ...
        reader.onload = async (e) => { 
          ...
          fetchFunction(flow)
            ...
            .catch((error : Error) => {
              alert("Unable to create the flow from file '" + file.name + "' : " + error.message)
            })
            .finally(() => {
          ...
          <label htmlFor="formId" className="">
              ...
              <div className="flex w-full h-full">
                <div className="flex flex-col justify-between grow bg-gray-700 border-6 border-solid">
                  ...
                  {selectedFiles.length && selectedFiles.map((file, id) => {
                    return (
                    <div className="text-white m-3" key={id}>{file.name}</div>
                  )})}
                  <input type="submit" className="m-3" />
                </div>
                <div
                  className={"flex flex-col items-center justify-center grow rounded border-4 border-dashed border-black dark:border-white"}
                  ...
                  {dragActive ? (
                    ...
                    <div className="text-black dark:text-white">Add files or drag and drop it...</div>
                    </>
                  ) : (
                    <>
